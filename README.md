...
python services/embed/main.py                 # :8090
npm --prefix apps/dashboard install && npm --prefix apps/dashboard run dev   # :3000
```
...
Config is environment variables: `THEORM_DB_ADDR`, `THEORM_LLAMA_URL`, `THEORM_EMBED_URL`.

## Status

Scaffold only. Build order is §12: **Phase A0 (spec compiler) first** — it removes
the cloud dependency from every later phase's tests.
